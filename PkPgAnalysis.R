...
#setwd("~/Dropbox/PKPG")
# Load Data ------------------------------------------------------------------------- #
Conc.raw <- read.table("/shared/silo_researcher/Gottardo_R/myajima_working/RExp/bppkgx/testdirectory/03_Data/Concentration_fixed.txt", header=T);
...
#
SNP  <- read.table("/shared/silo_researcher/Gottardo_R/myajima_working/RExp/bppkgx/testdirectory/03_Data/snp.txt", header = T) - 1;
SNP  <- SNP[,names(SNP)!="ID"]
#SNP[is.na(SNP)] <- 919.919;
SNP[is.na(SNP)] <- 0
# 
X    <- read.csv("/shared/silo_researcher/Gottardo_R/myajima_working/RExp/bppkgx/testdirectory/03_Data/covariate.csv", header=T); 
race <- rep(1, nrow(X));
...
#X1[is.na(X1)] <- 919.919;
#
T0     <- as.matrix(read.table("/shared/silo_researcher/Gottardo_R/myajima_working/RExp/bppkgx/testdirectory/03_Data/T01.txt", header=T));
...
library(bppkgx);
tpm <- proc.time();
workdir="/shared/silo_researcher/Gottardo_R/myajima_working/RExp/bppkgx/testdirectory/crossvalidation/test1"
dir.create(workdir); 
setwd(workdir)
